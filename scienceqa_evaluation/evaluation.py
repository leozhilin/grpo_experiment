...
def evaluate_model(rank, world_size, model_path):
    ...
    # Create directory for error samples
    error_dir = "./scienceqa_error_samples"
    ...
    for idx, example in enumerate(dataset):
        ...
        #         img.save(os.path.join(error_dir, f"{sample_idx}.png"))
        #     except Exception as e:
        #         print(f"Error saving image for sample {sample_idx}: {e}")
        #         print(f"Image data type: {type(image_data)}")

        #     # Save text information
        #     with open(os.path.join(error_dir, f"{sample_idx}.txt"), "w", encoding="utf-8") as f:
        ...
        #         f.write(f"Ground Truth: {choices[correct_option]}\n")
        #         f.write(f"Model Choice: {model_option}\n")
        #         f.write(f"Correct Choice: {correct_option}\n")
        #         f.write(f"Grade: {example['grade']}\n")
...
def main():
    model_paths = [
        ...
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-100",
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-200",
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-300",
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-400",
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-500",
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-600",
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-700",
        "models/ckpt/qwen2_2b_scienceqa_ckpt_cl/checkpoint-778",
    ]
